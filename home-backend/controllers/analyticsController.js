...
const getOverview = async (req, res, next) => {
  try {
    ...
    const expenseCategories = catAgg.map((c) => ({
      category: c._id,
      amount: round2(c.amount),
      percentage:
        totalExpense > 0 ? round2((c.amount / totalExpense) * 100) : 0,
      count: c.count,
    }));
    ...
    const budgetSummary = budgets.map((b) => ({
      ...
      percentage:
        b.limitAmount > 0 ? round2((b.spentAmount / b.limitAmount) * 100) : 0,
  ...
  try {
    ...
    for (let i = monthsBack - 1; i >= 0; i--) {
      ...
      trends.push({
        ...
        monthLabel: ref.toLocaleString("en-US", {
          month: "short",
          year: "numeric",
        }),
        income,
...
const getDailySpending = async (req, res, next) => {
  try {
    ...
    const monthsBack = Math.min(
      12,
      Math.max(1, parseInt(req.query.months) || 3),
    );
    const now = new Date();
...
const getBudgetAlerts = async (req, res, next) => {
  try {
    ...
    for (const b of budgets) {
      ...
      const overrunProb = estimateBudgetOverrunProbability(
        spent,
        limit,
        daysLeft,
      );
      ...
      if (status !== "ok" || overrunProb >= 0.6) {
        const estOverrun = Math.max(
          0,
          round2(
    ...
    alerts.sort(
      (a, b) =>
        severityOrder[a.prediction.severity] -
        severityOrder[b.prediction.severity],
    );
      ...
      VoiceFeedback.aggregate([
        { $match: { userId } },
        {
          $group: {
            _id: null,
            total: { $sum: 1 },
            corrected: { $sum: { $cond: ["$hadCorrection", 1, 0] } },
          },
        },
      ]),
    ...
    res.status(200).json({
      ...
      aiPerformance: {
        voice: {
          ...
          correctionRate:
            voiceTotal > 0 ? round2((voiceCorrect / voiceTotal) * 100) : null,
        },
        ocr: {
          usage: ocrCount,
          feedbackCount: ocrTotal,
          correctionRate:
            ocrTotal > 0 ? round2((ocrCorrect / ocrTotal) * 100) : null,
        },
      },
      timeSaved: {
  ...
  try {
    ...
    const goalsWithStatus = await Promise.all(
      goals.map(async (g) => {
        ...
        else if (probability < 0.3) goalStatus = "behind";
        else if (probability < 0.7) goalStatus = "at-risk";
